...
/**
 * A GraphStage that groups Row elements by their total serialized size in bytes.
 * Once the sum of sizes in the buffer reaches the specified `maxBatchSizeBytes`,
 * the batch is emitted and a new one starts.
 *
 * This version calls `row.toByteArray` inside the stage to measure each row's size.
 * If a single row itself exceeds the maximum, the stage fails.
 *
 * @param maxBatchSizeBytes the maximum allowed size in bytes for each emitted batch
 */
class SizeBasedBatcher(maxBatchSizeBytes: Long)
  extends GraphStage[FlowShape[Row, List[Row]]] {
    ...
    new GraphStageLogic(shape) {
      ...
      setHandler(in, new InHandler {
        override def onPush(): Unit = {
          ...
          // If adding this row would exceed the batch limit, emit the current batch first
          if (currentSize + elemSize > maxBatchSizeBytes) {
            emitBatch()
            buffer.clear()
            currentSize = 0L
          }
