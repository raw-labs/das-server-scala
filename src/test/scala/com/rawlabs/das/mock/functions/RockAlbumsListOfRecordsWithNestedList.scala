...
class RockAlbumsListOfRecordsWithNestedList extends DASMockFunction {
  ...
  private val albums: List[Album] = List(
    Album(
      ...
      "Pink Floyd",
      List("Speak to Me", "Breathe", "On the Run", "Time", "Money")
    ),
    Album(
      "Led Zeppelin IV",
      "Led Zeppelin",
   ...
   * The execute method.
   *
   * This function ignores any input arguments and returns a Value
   * containing a list of album records.
   */
    ...
    albums.foreach { album =>
      ...
      recordBuilder.addAtts(
        ValueRecordAttr.newBuilder()
          ...
          .setValue(
            Value.newBuilder()
              .setString(
                ValueString.newBuilder().setV(album.albumName).build()
              )
          )
          .build()
      )
      ...
      recordBuilder.addAtts(
        ValueRecordAttr.newBuilder()
          ...
          .setValue(
            Value.newBuilder()
              .setString(
                ValueString.newBuilder().setV(album.band).build()
              )
          )
          .build()
      )
      ...
      album.songs.foreach { song =>
        songsListBuilder.addValues(
          Value.newBuilder()
            .setString(
              ValueString.newBuilder().setV(song).build()
            )
        )
      }
      recordBuilder.addAtts(
        ValueRecordAttr.newBuilder()
          .setName("songs")
          .setValue(
            Value.newBuilder().setList(songsListBuilder.build())
          )
          .build()
      )
   ...
   *
   * This declares a function "rock_albums" that takes no parameters and returns
   * a list of records. Each record contains:
  ...
  def definition: FunctionDefinition = {
    ...
    val albumRecordType = Type.newBuilder()
      .setRecord(
    ...
    val returnType = Type.newBuilder()
      .setList(
        ListType.newBuilder()
    ...
    FunctionDefinition.newBuilder()
      .setFunctionId(
        FunctionId.newBuilder().setName("rock_albums").build()
      )
